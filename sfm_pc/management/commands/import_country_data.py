...
from membershiporganization.models import (
...
class Command(BaseCommand):
    help = "Import data from local directory"

    def add_arguments(self, parser):
        parser.add_argument(
            "--country_code", dest="country_code", help="Country code for the import"
        )
        parser.add_argument(
            "--country_path",
            dest="country_path",
    ...
    def handle(self, *args, **options):
        ...
        all_sheets = self.get_sheets_from_folder(country_path)
        ...
        for entity_type in options["entity_types"].split(","):
            ...
            for title, sheet in sheets.items():
                self.stdout.write(
                    self.style.SUCCESS(
                        "Creating {0}s from {1} ... ".format(entity_type, title)
                    )
                )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "Successfully imported data from {}".format(options["country_path"])
            )
        )
    ...
    def log_conflicts(self, entity_type, entity_map, transpose=False):
        ...
        for key, values in entity_map.get_conflicts(transpose=transpose):
            for value, locations in values.items():
                for column, row, sheet in sorted(
                    locations, key=lambda location: location[1]
                ):
    ...
    def log_error(self, message, sheet=None, current_row=None):
        ...
        file_name_with_country_code = (
            file_name
            if self.country_code in file_name
    ...
    def create_organization(self, org_data):
        ...
        org_positions = {
            "Name": {
                ...
                "confidence": Organization.get_spreadsheet_confidence_field_name(
                    "name"
                ),
            ...
            "Classification": {
                ...
                "source": Organization.get_spreadsheet_source_field_name(
                    "classification"
                ),
            },
            "DivisionId": {
            ...
            "FirstCitedDate": {
                ...
                "day": Organization.get_spreadsheet_field_name("firstciteddate")
                + "_day",
                "month": Organization.get_spreadsheet_field_name("firstciteddate")
                + "_month",
                "year": Organization.get_spreadsheet_field_name("firstciteddate")
                + "_year",
                "confidence": Organization.get_spreadsheet_confidence_field_name(
                    "firstciteddate"
                ),
            ...
            "LastCitedDate": {
                ...
                "day": Organization.get_spreadsheet_field_name("lastciteddate")
                + "_day",
                "month": Organization.get_spreadsheet_field_name("lastciteddate")
                + "_month",
                "year": Organization.get_spreadsheet_field_name("lastciteddate")
                + "_year",
                "confidence": Organization.get_spreadsheet_confidence_field_name(
                    "lastciteddate"
                ),
        ...
        composition_positions = {
            "Parent": {
                ...
                "confidence": Composition.get_spreadsheet_confidence_field_name(
                    "parent"
                ),
            ...
            "Classification": {
                ...
                "source": Composition.get_spreadsheet_source_field_name(
                    "classification"
                ),
            },
            "StartDate": {
        ...
        membership_positions = {
            "OrganizationOrganization": {
                "value": MembershipOrganization.get_spreadsheet_field_name(
                    "organization"
                ),
                "confidence": MembershipOrganization.get_spreadsheet_confidence_field_name(
                    "organization"
                ),
                "source": MembershipOrganization.get_spreadsheet_source_field_name(
                    "organization"
                ),
            },
            "FirstCitedDate": {
                ...
                "day": MembershipOrganization.get_spreadsheet_field_name(
                    "firstciteddate"
                )
                + "_day",
                "month": MembershipOrganization.get_spreadsheet_field_name(
                    "firstciteddate"
                )
                + "_month",
                "year": MembershipOrganization.get_spreadsheet_field_name(
                    "firstciteddate"
                )
                + "_year",
                "confidence": MembershipOrganization.get_spreadsheet_confidence_field_name(
        ...
        if admin_status != "3":
            self.stdout.write(
                'Skipping unit "{}" because its status is {}'.format(
                    name_value, admin_status
                )
            )
            return None
        ...
        if confidence and source:
            ...
            if confidence and sources:
                ...
                try:
                    ...
                    org_info["Organization_OrganizationName"][
                        "sources"
                    ] += existing_sources
                ...
                for attr in org_attributes:
                    self.make_relation(
                        attr, org_positions[attr], org_data, organization
                    )
                ...
                if org_data[location_positions["Location"]["type"]] == "aoo":
                    ...
                    if humane_id:
                        self.make_area(humane_id, org_data, organization)
                    ...
                    if site_osm_id:
                        self.make_emplacement(site_osm_id, org_data, organization)

                if org_data["unit:relation_type"] == "child":
                    # Create Compositions

                    try:
                        parent_org_name = org_data[
                    ...
                    if parent_org_name:
                        try:
                            parent_confidence = self.get_confidence(
                                org_data[composition_positions["Parent"]["confidence"]]
                            )
                        except (IndexError, KeyError):
                            self.log_error(
                                "Parent organization for {} does not have confidence".format(
                                    organization.name
                                )
                            )
                        ...
                        try:
                            parent_sources = self.get_sources(
                                org_data[composition_positions["Parent"]["source"]]
                            )
                        except IndexError:
                            self.log_error(
                                "Parent organization for {} does not have a source".format(
                                    organization.name
                                )
                            )
                            parent_sources = None
                        ...
                        if parent_confidence and parent_sources:
                            ...
                            try:
                                ...
                                self.log_error(
                                    'Invalid parent unit UUID: "{}"'.format(uuid)
                                )
                                return None
                            else:
                                existing_sources = self.sourcesList(
                                    parent_organization, "name"
                                )
                                parent_org_info["Organization_OrganizationName"][
                                    "sources"
                                ] += existing_sources
                            ...
                            try:
                                composition = Composition.objects.get(
                                    compositionparent__value=parent_organization,
                            ...
                            self.log_error(
                                "Parent organization for {} does not have source or confidence".format(
                                    organization.name
                                )
                            )
                    ...
                    if member_org_name:
                        try:
                            confidence = self.get_confidence(
                                org_data[
                                    membership_positions["OrganizationOrganization"][
                                        "confidence"
                                    ]
                                ]
                            )
                            ...
                            self.log_error(
                                "Member organization for {} does not have confidence".format(
                                    member_org_name
                                )
                            )
                            confidence = None
                        ...
                        try:
                            ...
                            self.log_error(
                                "Member organization for {} does not have a source".format(
                                    member_org_name
                                )
                            )
                            sources = None
                        ...
                        if confidence and sources:
                            ...
                            try:
                                member_organization = Organization.objects.get(
                                    uuid=uuid
                                )
                            except Organization.DoesNotExist:
                                member_organization = Organization.objects.create(
                                    uuid=uuid, published=True
                                )
                            except ValidationError:
                                self.log_error(
                                    'Invalid member unit UUID: "{}"'.format(uuid)
                                )
                                return None
                            else:
                                existing_sources = self.sourcesList(
                                ...
                                member_org_info["Organization_OrganizationName"][
                                    "sources"
                                ] += existing_sources
                            ...
                            try:
                                date_parts = [
                                    org_data[
                                        membership_positions["FirstCitedDate"]["year"]
                                    ],
                                    org_data[
                                        membership_positions["FirstCitedDate"]["month"]
                                    ],
                                    org_data[
                                        membership_positions["FirstCitedDate"]["day"]
                                    ],
                                ]
                                fcd = self.parse_date(
                                    "-".join(filter(None, date_parts))
                                )
                            except IndexError:
                                fcd = None
                            ...
                            try:
                                date_parts = [
                                    org_data[
                                        membership_positions["LastCitedDate"]["year"]
                                    ],
                                    org_data[
                                        membership_positions["LastCitedDate"]["month"]
                                    ],
                                    org_data[
                                        membership_positions["LastCitedDate"]["day"]
                                    ],
                                ]
                                lcd = self.parse_date(
                                    "-".join(filter(None, date_parts))
                            ...
                            try:
                                membership = MembershipOrganization.objects.get(
                                    membershiporganizationmember__value=organization,
                                ...
                                sources = set(
                                    self.sourcesList(membership, "member")
                                ...
                                membership = MembershipOrganization.create(
                                    membership_info
                                )
                            ...
                            self.log_error(
                                "Member organization for {} does not have source or confidence".format(
                                    member_org_name
                                )
                            )

                self.stdout.write(
                    self.style.SUCCESS("Created {}".format(organization.get_value()))
                )

            else:
                self.log_error(
                    "{} did not have a confidence or source".format(name_value)
                )
            ...
            self.log_error(
                "{0} did not have {1}".format(name_value, ", ".join(missing))
            )
    ...
    def make_relation(
        ...
        if required and not value:
            self.log_error(
                "No {0} information for {1}".format(field_name, instance.get_value())
            )
            return None
            ...
            relation_path = import_path.format(
                app_name=app_name, model_name=model_name, field_name=field_name
            )
            ...
            if (sources or not source_required) and (
                confidence or not confidence_required
            ):
                if isinstance(
                    relation_model._meta.get_field("value"), models.ForeignKey
                ):
                    value_rel_path_fmt = "{app_name}.models.{field_name}"
                    value_rel_path = value_rel_path_fmt.format(
                        app_name=app_name, field_name=field_name
                    )
                    ...
                    try:
                        ...
                        value_rel_path = value_rel_path_fmt.format(
                            app_name=app_name, field_name="Context"
                        )
                        value_model = import_class(value_rel_path)
                    ...
                    values = (
                        [value]
                        if not multiple
                        else [val.strip() for val in value.split(";") if val.strip()]
                    )
                    for value_text in values:
                        value_obj, created = value_model.objects.get_or_create(
                            value=value_text
                        )
                        ...
                        (
                            ...
                            value=value_obj, object_ref=instance, lang="en"
                        )
                        if created:
                    ...
                    relation_model._meta.get_field("value"), ApproximateDateField
                ):
                    ...
                    if parsed_value:
                        (
                            ...
                            value=parsed_value, object_ref=instance, lang="en"
                        )
                        if created:
                            for source in sources:
                                relation_instance.accesspoints.add(source)
                        ...
                        self.log_error(
                            "Expected a date for {app_name}.models.{field_name} but got {value}".format(
                                app_name=app_name, field_name=field_name, value=value
                            )
                        )
                        return None
                    ...
                    for val in values:
                        (
                            ...
                            value=val.strip(), object_ref=instance, lang="en"
                        )
                        if created:
                            for source in sources:
                                relation_instance.accesspoints.add(source)
                ...
                message = "{field_name} from {app_name}.{model_name} does not have {missing}".format(
    ...
    def make_area(self, humane_id, org_data, organization):
        ...
        relation_positions = {
            "StartDate": {
                ...
                "confidence": Association.get_spreadsheet_confidence_field_name(
                    "startdate"
                ),
        ...
        try:
            ...
            self.log_error(
                'Location "{0}" for Area for {1} does not exist'.format(
                    humane_id, organization.name
                )
            )
            return None

        try:
            area_confidence = self.get_confidence(
                org_data[positions["Location"]["confidence"]]
            )
        except (IndexError, KeyError):
            self.log_error(
                "Location for Area for {} does not have confidence".format(
                    organization.name
                )
            )
            return None
        ...
        try:
            ...
            self.log_error(
                "Location for Area for {} does not have source".format(
                    organization.name
                )
            )
            return None
        ...
        try:
            assoc = Association.objects.get(
                associationorganization__value=organization, associationarea__value=area
            )
        ...
        for field_name, positions in relation_positions.items():
            if field_name == "StartDate":
                startdate = self.make_relation(
                    field_name, positions, org_data, assoc, date=True
                )

            elif field_name == "EndDate":
                enddate = self.make_relation(
                    field_name, positions, org_data, assoc, date=True
                )
                ...
                self.make_relation(field_name, positions, org_data, assoc)

        association_tenure, _ = AssociationTenure.objects.get_or_create(
            association=assoc, startdate=startdate, enddate=enddate
        )

    def make_emplacement(self, humane_id, org_data, organization):
        positions = {
            "Location": {
                "value": "unit:location",
                "confidence": "unit:location:confidence",
        ...
        relation_positions = {
            "StartDate": {
                ...
                "confidence": Emplacement.get_spreadsheet_confidence_field_name(
                    "startdate"
                ),
        ...
        try:
            ...
            self.log_error(
                'Location "{0}" for Site for {1} does not exist'.format(
                    humane_id, organization.name
                )
            )
            return None
        ...
        if sources and confidence:
            ...
            try:
                emplacement = Emplacement.objects.get(
                    emplacementorganization__value=organization,
            ...
            for field_name, positions in relation_positions.items():
                if field_name == "StartDate":
                    startdate = self.make_relation(
                        field_name, positions, org_data, emplacement, date=True
                    )
                    ...
                    self.make_relation(field_name, positions, org_data, emplacement)

            emplacement_tenure, _ = EmplacementTenure.objects.get_or_create(
                emplacement=emplacement, startdate=startdate, enddate=enddate
            )
            ...
            self.log_error(
                "Emplacement for org {0} did not have {1}".format(
                    organization.name.get_value().value, ", ".join(missing)
                )
            )
            return None
    ...
    def make_real_date(self, *, data, position, model, attribute, object_ref):
        ...
        instance, created = model.objects.get_or_create(
            value=real_date, object_ref=object_ref
        )
    ...
    def create_person(self, person_data):
        ...
        person_positions = {
            ...
            "DivisionId": {
                ...
                "confidence": Person.get_spreadsheet_confidence_field_name(
                    "division_id"
                ),
        ...
        membership_positions = {
            ...
            "Role": {
                ...
                "confidence": MembershipPerson.get_spreadsheet_confidence_field_name(
                    "role"
                ),
            ...
            "FirstCitedDate": {
                ...
                "day": MembershipPerson.get_spreadsheet_field_name("firstciteddate")
                + "_day",
                "month": MembershipPerson.get_spreadsheet_field_name("firstciteddate")
                + "_month",
                "year": MembershipPerson.get_spreadsheet_field_name("firstciteddate")
                + "_year",
                "confidence": MembershipPerson.get_spreadsheet_confidence_field_name(
                    "firstciteddate"
                ),
                "source": MembershipPerson.get_spreadsheet_source_field_name(
                    "firstciteddate"
                ),
            },
            "RealStart": {
            ...
            "LastCitedDate": {
                ...
                "day": MembershipPerson.get_spreadsheet_field_name("lastciteddate")
                + "_day",
                "month": MembershipPerson.get_spreadsheet_field_name("lastciteddate")
                + "_month",
                "year": MembershipPerson.get_spreadsheet_field_name("lastciteddate")
                + "_year",
                "confidence": MembershipPerson.get_spreadsheet_confidence_field_name(
                    "lastciteddate"
                ),
        ...
        if admin_status != "3":
            self.stdout.write(
                'Skipping person "{}" because its status is {}'.format(
                    name_value, admin_status
                )
            )
            return None
        ...
        if confidence and source:
            ...
            if confidence and sources:
                ...
                self.make_relation(
                    "Alias", person_positions["Alias"], person_data, person
                )
                ...
                try:
                    confidence = self.get_confidence(
                        person_data[membership_positions["Organization"]["confidence"]]
                    )
                except (KeyError, IndexError):
                    self.log_error(
                        "Person {0} as a member of {1} has no confidence".format(
                            name_value, organization_name
                        )
                    )
                    return None
                ...
                try:
                    sources = self.get_sources(
                        person_data[membership_positions["Organization"]["source"]]
                    )
                except (KeyError, IndexError):
                    self.log_error(
                        "Person {0} as a member of {1} has no sources".format(
                            name_value, organization_name
                        )
                    )
                    return None
                ...
                org_info["Organization_OrganizationDivisionId"][
                    "sources"
                ] += div_sources
                ...
                try:
                    role_name = person_data[
                        membership_positions["Role"]["value"]
                    ].strip()
                except IndexError:
                    role = None
                else:
                ...
                try:
                    rank_name = person_data[
                        membership_positions["Rank"]["value"]
                    ].strip()
                except IndexError:
                    rank = None
                else:
                ...
                try:
                    title = (
                        person_data[membership_positions["Title"]["value"]].strip()
                        or None
                    )
                except IndexError:
                    title = None
                ...
                try:
                    ...
                    membership_data["MembershipPerson_MembershipPersonMember"][
                        "sources"
                    ] += sources
                ...
                self.make_relation(
                    "Role", membership_positions["Role"], person_data, membership
                )

                self.make_relation(
                    "Title", membership_positions["Title"], person_data, membership
                )

                self.make_relation(
                    "Rank", membership_positions["Rank"], person_data, membership
                )
                ...
                self.stdout.write(
                    self.style.SUCCESS("Created {}".format(person.get_value()))
                )
    ...
    def create_person_extra(self, data):
        ...
        biography_positions = {
            "Gender": {
                ...
                "confidence": PersonBiography.get_spreadsheet_confidence_field_name(
                    "gender"
                ),
            ...
            "DateOfBirth": {
                ...
                "source": PersonBiography.get_spreadsheet_source_field_name(
                    "date_of_birth"
                ),
            },
            "Deceased": {
        ...
        extra_positions = {
            "AccountType": {
                ...
                "confidence": PersonExtra.get_spreadsheet_confidence_field_name(
                    "account_type"
                ),
            ...
            "ExternalLinkDescription": {
                ...
                "value": PersonExtra.get_spreadsheet_field_name(
                    "external_link_description"
                ),
                "confidence": PersonExtra.get_spreadsheet_confidence_field_name(
                    "external_link_description"
                ),
                "source": PersonExtra.get_spreadsheet_source_field_name(
                    "external_link_description"
                ),
            },
            "MediaDescription": {
        ...
        self.stdout.write(self.style.SUCCESS("Working on {}".format(person_name_value)))
            ...
            def get_info_from_positions(positions):
                ...
                for positions in positions.values():
                    if positions.get("key"):
                        info[positions["key"]] = {"value": data[positions["value"]]}
    ...
    def create_event(self, event_data):
        positions = {
            ...
            "PerpetratorOrganization": {
                "value": Violation.get_spreadsheet_field_name(
                    "perpetratororganization"
                ),
                "source": "incident:all:source",
            },
        ...
        if admin_status != "3":
            self.stdout.write(
                'Skipping event "{}" because its status is {}'.format(
                    uuid, admin_status
                )
            )
            return None
        ...
        for attr in simple_multi_attrs:
            self.make_relation(
                attr, positions[attr], event_data, violation, require_confidence=False
            )
        ...
        try:
            ...
            self.log_error(
                "Location {} for ViolationLocation does not exist".format(humane_id)
            )
            self.log_error("Country code missing")
            return None
            ...
            geo, admin1, admin2 = (
                exact_location,
                exact_location.adminlevel1,
                exact_location.adminlevel2,
            )
        ...
        if perpetrator:
            ...
            for perp in perps:
                ...
                vp, created = ViolationPerpetrator.objects.get_or_create(
                    value=person, object_ref=violation
                )
                if created:
                    with reversion.create_revision():
                        for source in sources:
        ...
        if perp_org:
            ...
            for org in orgs:
                ...
                (
                    vpo_obj,
                    created,
                ) = ViolationPerpetratorOrganization.objects.get_or_create(
                    value=organization, object_ref=violation
                )
