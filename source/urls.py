...
from source.views import get_sources, SourceCreate, source_autocomplete, \
    ...
    AccessPointUpdate, AccessPointCreate
...
urlpatterns = [
    ...
    url(r'^update-access-points/(?P<source_id>[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})/(?P<pk>\d+)/$', AccessPointUpdate.as_view(), name="update-access-points"),
